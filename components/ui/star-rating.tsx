  ...
  ({
    ...
    const { stars, tooltipText } = useMemo(() => {
      ...
      for (let i = 0; i < validFullStars; i++) {
        starsArray.push(
          <span key={`full-${i}`} style={{ color: starColor }}>
            ★
          </span>,
        );
      }
      ...
      if (validHasHalfStar) {
        ...
        starsArray.push(
          <span key="half" style={halfStarStyle}>
            ★
          </span>,
        );
      }
      ...
      if (rating <= 0 && maxRating > 0) {
        ...
        for (let i = 0; i < maxRating; i++) {
          starsArray.push(
            <span key={`zero-empty-${i}`} style={{ color: "#ccc" }}>
              ☆
            </span>,
          );
        }
    ...
    const tooltipElement = (
      <Tooltip>
        <TooltipTrigger asChild>{starsContent}</TooltipTrigger>
        <TooltipContent>
          <p>{tooltipText}</p>
        </TooltipContent>
      </Tooltip>
    ...
    return (
      <TooltipProvider delayDuration={100}>{tooltipElement}</TooltipProvider>
    );
  },
);
